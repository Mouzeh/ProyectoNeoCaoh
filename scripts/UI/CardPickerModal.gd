...
# ============================================================
# CONSTRUCTOR DINÁMICO (Dibuja su propia UI)
# ============================================================
func _init() -> void:
	layer = 100 # Para que quede por encima de todo el juego
	...
	# 6. Grilla para las cartas
	_grid = GridContainer.new()
	_grid.columns = 5 # 5 cartas por fila
...
func setup(title_text: String, cards: Array, min_picks: int = 1, max_picks: int = 1) -> void:
	get_node("Panel/VBoxContainer/TitleLabel").text = title_text
	_cards_array = cards
...
func _populate_grid() -> void:
	...
	for i in range(_cards_array.size()):
		...
		# Overlay verde para cuando la seleccionas
		var highlight = ColorRect.new()
		highlight.name = "Highlight"
		highlight.color = Color(0.2, 0.9, 0.2, 0.5)
...
func _on_card_clicked(idx: int, wrapper: Control) -> void:
	var hl = wrapper.get_node("Highlight")
	...
	if _selected_indices.has(idx):
		...
		if _selected_indices.size() >= _max_picks:
			if _max_picks == 1:
				# Si solo se puede elegir 1, cambiamos la selección antigua por la nueva automáticamente
				var old_idx = _selected_indices[0]
				_selected_indices.clear()
				_grid.get_child(old_idx).get_node("Highlight").visible = false
				_selected_indices.append(idx)
				hl.visible = true
			else:
				# Si ya llegaste al máximo (ej. 2 o 3), no hace nada
				return
		else:
			_selected_indices.append(idx)
			hl.visible = true
